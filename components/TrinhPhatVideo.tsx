...
export default function TrinhPhatVideo({ phim, isActive, onClose }: { 
  ...
  // ==================== DETECT MOBILE (quan trọng cho PWA) ====================
  ...
  // ==================== HLS INIT - ĐÃ FIX HOÀN TOÀN CHO MOBILE + PWA ====================
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      const hlsConfig = {
        ...
        enableWorker: !isMobile,                    // TẮT WORKER TRÊN MOBILE
  ...
  // ==================== AUTO RECOVER STALLED + VISIBILITY (PWA) ====================
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
