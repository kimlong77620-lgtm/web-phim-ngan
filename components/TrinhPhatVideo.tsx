...
        onStalled={() => console.warn('Native stalled')}
        onWaiting={() => console.warn('Native waiting')}
      />
      
...
          {/* ==================== KHUNG CHỌN CHẤT LƯỢNG (ĐÃ TRỞ LẠI) ==================== */}
          {qualities.length > 0 && (
...
                        setShowQualityMenu(false); 
                      }} 
                      className={`text-[11px] font-bold py-1.5 px-2 rounded transition-colors ${currentQuality === q.index ? 'bg-yellow-500 text-black' : 'text-white hover:bg-white/20'}`}
                    >
                      {q.label}
                    </button>
                  ))}
...
                  setShowVolumeSlider(false); 
                }} 
                className="w-10 h-10 flex items-center justify-center bg-black/50 rounded-full text-white hover:bg-yellow-500 hover:text-black transition-colors backdrop-blur-md shadow-lg font-bold text-[9px] uppercase"
              >
