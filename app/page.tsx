...
// 2. PHÒNG CHIẾU (GIỮ MÓNG + TÍCH HỢP CẢM BIẾN VUỐT THOÁT)
function PhongChieu({ phim: initialPhim, onClose, danhSachToanBo }: { phim: Phim; onClose: () => void; danhSachToanBo: Phim[]; }) {
  ...
  const minSwipeDistance = 70; // Độ nhạy vuốt

  // 🎯 Tối ưu hàm đóng phim bằng useCallback
  const handleClose = useCallback(() => {
    window.history.replaceState(null, '', window.location.pathname);
    onClose();
  }, [onClose]);

  // Cuộn đến phim được chọn khi mới mở
  useEffect(() => {
    const el = document.getElementById(`snap-video-${initialPhim.id}`);
    if (el) el.scrollIntoView({ behavior: 'instant' });
  }, [initialPhim.id]);

  // Bộ nhận diện đang xem phim nào (Tối ưu hiệu suất)
  useEffect(() => {
    ...
    // Chỉ observe sau khi render xong
  ...
  const onTouchEnd = () => {
    ...
    // Vuốt từ trái sang phải (như back trên iOS) hoặc Vuốt từ trên xuống
  ...
  return (
    <div 
      className="absolute top-0 left-0 w-full h-[100dvh] bg-black z-50 overflow-y-scroll snap-y snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
      onTouchStart={onTouchStart}
      onTouchMove={onTouchMove}
      onTouchEnd={onTouchEnd}
    >
      {/* 🎯 NÚT BACK DỰ PHÒNG CHO PC (hoặc khách không biết vuốt) */}
      <button 
        onClick={handleClose}
        className="fixed top-6 left-6 z-[9999] p-3 bg-black/40 hover:bg-yellow-500 text-white hover:text-black rounded-full backdrop-blur-md border border-white/20 transition-all shadow-2xl active:scale-90"
      >
...
export default function Home() {
  ...
  const handleAutoPayment = async () => {
    try {
      ...
      const res = await fetch("/api/create-payment", {
        ...
        body: JSON.stringify({ fanId: fanId, amount: 45000 }),
      });
      
      const data = await res.json();
      if (data.bin) {
        window.location.href = data.bin;
      } else {
  ...
  useEffect(() => {
    const check = async () => {
      // 🎯 Fix lỗi .single() bằng .limit(1).single() để chống sập web
            ...
            {paymentStep === 1 ? (
              ...
              <div className="text-center space-y-4">
                <div className="bg-white p-2 rounded-2xl inline-block shadow-xl">
                  <img src={`https://img.vietqr.io/image/VCB-1042526602-compact2.jpg?amount=45000&addInfo=DONATE%5F${fanId}`} className="w-40 h-40" alt="QR Vietcombank" />
                </div>
