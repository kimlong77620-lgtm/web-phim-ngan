...
// 1. KIỂU DỮ LIỆU (Đã gia cố các cột đếm)
interface Phim {
  id: number;
  title: string;
  thumb: string;
...
function PhongChieu({ phim: initialPhim, onClose, danhSachToanBo }: { phim: Phim; onClose: () => void; danhSachToanBo: Phim[]; }) {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll('.snap-video-item').forEach(el => observer.observe(el));
    ...
    <div className="absolute top-0 left-0 w-full h-[100dvh] bg-black z-50 overflow-y-scroll snap-y snap-mandatory scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      {danhSachToanBo.map((phimItem) => (
        <div key={phimItem.id} id={`snap-video-${phimItem.id}`} data-id={phimItem.id} className="snap-video-item relative w-full h-[100dvh] snap-start snap-always shrink-0 bg-black">
          {/* Truyền phim vào máy chiếu với chốt bảo vệ phimItem */}
          {phimItem && (
            <TrinhPhatVideo 
              phim={phimItem} 
              isActive={activeId === phimItem.id} 
              onClose={() => { window.history.replaceState(null, '', window.location.pathname); onClose(); }} 
            />
          )}
        </div>
      ))}
...
// 3. TRANG CHỦ (TỔNG LỰC TRANG TRÍ)
export default function Home() {
  ...
  // 🚀 CÔNG TẮC THANH TOÁN PAYOS TỰ ĐỘNG
  const handleAutoPayment = async () => {
    try {
      setLoadingPhim(true); // Mượn tạm hiệu ứng tải sạp của tiên sinh
      const res = await fetch("/api/create-payment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fanId: fanId, amount: 45000 }), // Giá gói VIP 45k của lão bản
      });
      
      const data = await res.json();
      if (data.bin) {
        window.location.href = data.bin; // Dẫn khách thẳng tới trang quét mã QR của PayOS
      } else {
        alert("Lỗi: " + (data.error || "Không thể tạo link thanh toán"));
      }
    } catch (err) {
      alert("Cổng thanh toán đang bận, lão bản vui lòng thử lại sau!");
    } finally {
  ...
  // LOAD DỮ LIỆU SẠP PHIM (Có check null an toàn)
  useEffect(() => {
  ...
  const handleCopy = (t: string, f: string) => {
    navigator.clipboard.writeText(t); setCopiedField(f); setTimeout(() => setCopiedField(""), 2000); 
  };
  ...
  useEffect(() => {
    const check = async () => {
      const { data: s } = await supabase.from('settings').select('is_experience_mode').single();
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#0b0f19] flex flex-col items-center justify-center p-6 text-center">
      ...
      <img src="/logo.jpg" className="w-40 h-40 rounded-full border-4 border-yellow-500 mb-12 shadow-2xl" />
      <h1 className="text-5xl md:text-7xl font-[900] text-yellow-500 uppercase italic mb-8" style={{ fontFamily: "'Lexend', sans-serif" }}>Xem Phim<br />Không Cần Não</h1>
      <SignInButton mode="modal"><button className="w-full max-w-sm rounded-2xl bg-yellow-500 py-5 text-2xl font-black text-black uppercase shadow-lg">🚀 Đăng nhập ngay</button></SignInButton>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#0b0f19] text-white p-4 pb-20 relative">
      ...
      <header className="sticky top-0 z-40 bg-[#0b0f19]/95 backdrop-blur-md pt-4 pb-4 px-4 border-b border-gray-800 shadow-xl mb-6">
        <div className="max-w-4xl mx-auto flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <img src="/logo.jpg" className="w-12 h-12 rounded-full border-2 border-yellow-500" />
            <h1 className="text-xl md:text-2xl font-black text-yellow-500 uppercase italic leading-tight">Xem Phim <br/> Không Cần Não</h1>
          </div>
        ...
        <div className="max-w-4xl mx-auto flex gap-2 relative z-50">
          <div ref={menuRef} className="relative flex-none">
            <button onClick={() => setShowMenu(!showMenu)} className="bg-gray-800 text-yellow-500 px-4 rounded-2xl border border-gray-700 h-12 flex items-center gap-2 font-bold transition-all active:scale-95"><span>☰</span> <span className="hidden sm:inline uppercase">Danh mục</span></button>
            {showMenu && (
              <div className="absolute top-full left-0 mt-3 w-60 bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl z-[100] overflow-hidden animate-in fade-in slide-in-from-top-2">
                ...
                {["Tất cả", "Hệ Thống", "Trọng Sinh", "Xuyên Sách", "Cổ Trang", "Ngôn Tình"].map((item, idx) => (
          ...
          <div className="relative flex-1 group">
            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-yellow-500">🔍</span>
            <input type="text" placeholder="Tìm tên phim, diễn viên..." value={tuKhoa} onChange={(e) => setTuKhoa(e.target.value)} className="w-full h-12 bg-gray-900 border border-gray-700 rounded-2xl pl-11 pr-4 outline-none focus:border-yellow-500 text-sm font-medium" />
          </div>
        </div>
      </header>

      <div className="max-w-4xl mx-auto">
        {/* 🕒 MỤC CÁ NHÂN (LỊCH SỬ XEM) */}
        {lichSuXem.length > 0 && !tuKhoa && (
          <div className="mb-10 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {lichSuXem.map((p) => (
                <div key={`hist-${p.id}`} onClick={() => handleWatchPhim(p)} className="group cursor-pointer bg-gray-900 rounded-xl overflow-hidden border border-blue-900/30 hover:border-blue-500 transition-all shadow-lg">
                  <div className="relative aspect-[2/3] overflow-hidden">
                    ...
                    <div className="absolute bottom-2 left-2 bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-md flex items-center gap-1 border border-white/10">
                      ...
                      <span className="text-[10px] font-bold uppercase">
                        {(p.views_count || 0) >= 1000 ? ((p.views_count || 0) / 1000).toFixed(1).replace('.0', '') + 'k' : p.views_count || 0}
                      </span>
                    </div>
                  </div>
        ...
        {/* 🎬 KHO PHIM MỚI */}
        <h2 className="text-lg font-bold mb-4 border-l-4 border-yellow-500 pl-2 uppercase italic tracking-tighter">Kho Phim Mới</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {phimHienThi.map((p) => (
            <div key={p.id} onClick={() => handleWatchPhim(p)} className="group cursor-pointer bg-gray-900 rounded-xl overflow-hidden border border-gray-800 hover:border-yellow-500 transition-all shadow-lg">
              <div className="relative aspect-[2/3] overflow-hidden">
                <img src={p.thumb} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
      ...
      {/* VIP MODAL - NÂNG CẤP PAYOS TỰ ĐỘNG */}
      {showVipModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[9999] p-4 backdrop-blur-md">
          <div className="bg-[#1a1f2e] p-8 rounded-3xl max-w-sm w-full text-center border-2 border-yellow-500 shadow-2xl relative animate-in zoom-in-95">
            ...
            <h2 className="text-3xl font-bold text-yellow-500 mb-6 uppercase italic">Xem Phim <br/> Không Cần Não</h2>
            ...
            {paymentStep === 1 ? (
              <div className="text-left space-y-4">
                ...
                {/* 🚀 NÚT CHÍNH: PAYOS TỰ ĐỘNG */}
                <button 
                  onClick={handleAutoPayment} 
                  className="w-full py-4 bg-yellow-600 text-black font-[900] rounded-2xl hover:bg-yellow-500 uppercase shadow-lg shadow-yellow-500/20 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                  🚀 Thanh toán tự động
                </button>

                {/* 🏦 NÚT PHỤ: CHUYỂN KHOẢN THỦ CÔNG */}
                <button 
                  onClick={() => setPaymentStep(2)} 
                  className="w-full py-2 text-gray-400 text-[11px] font-bold uppercase hover:text-white transition-colors border border-gray-700 rounded-xl"
                >
                  Hoặc chuyển khoản thủ công
                </button>
              </div>
            ) : (
              /* GIỮ NGUYÊN VIETCOMBANK CHO TIÊN SINH LÀM DỰ PHÒNG */
              <div className="text-center space-y-4">
                <div className="bg-white p-2 rounded-2xl inline-block shadow-xl">
                  <img src={`https://img.vietqr.io/image/VCB-1042526602-compact2.jpg?amount=45000&addInfo=DONATE%5F${fanId}`} className="w-40 h-40" />
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={() => window.location.reload()} className="flex-[2] py-3 bg-yellow-600 text-black font-black rounded-xl uppercase text-xs hover:bg-yellow-500 shadow-lg transition-all active:scale-95">Tôi đã chuyển khoản</button>
                </div>
              </div>
            )}
          </div>
