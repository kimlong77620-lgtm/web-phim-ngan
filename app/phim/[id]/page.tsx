  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (event, data) => {
        ...
        const seen = new Set();
        for (const q of raw) { 
          if (!seen.has(q.label)) { filtered.push(q); seen.add(q.label); } 
        }
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-black flex justify-center items-center overflow-hidden" onMouseMove={resetTimer} onTouchStart={resetTimer}>
      ...
      <div className={`absolute inset-0 z-30 pointer-events-none transition-opacity duration-500 ${showUI ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="pointer-events-auto absolute right-4 bottom-36 z-[100] flex flex-col gap-5 items-center">
          ...
          {danhSachTap.length > 1 && (
            <div className="relative flex items-center justify-center">
              {showTapMenu && (
                <div className="absolute right-full mr-3 bg-black/80 rounded-xl p-2 flex flex-col gap-1 backdrop-blur-md shadow-[0_0_15px_rgba(0,0,0,0.5)] min-w-[100px] max-h-[200px] overflow-y-auto">
                  {danhSachTap.map((_, index) => (
                    <button 
                      ...
                      className={`text-[11px] font-bold py-1.5 px-2 rounded transition-colors ${tapHienTai === index ? 'bg-red-500 text-white' : 'text-white hover:bg-white/20'}`}
                    >
                      Tập {index + 1}
                    </button>
                  ))}
              ...
              <button 
                onClick={(e) => { 
                  e.stopPropagation(); 
                  setShowTapMenu(!showTapMenu); 
                  setShowQualityMenu(false); 
                  setShowVolumeSlider(false); 
                }} 
                className={`w-10 h-10 flex flex-col items-center justify-center rounded-full text-white transition-colors backdrop-blur-md shadow-lg ${showTapMenu ? 'bg-red-500' : 'bg-black/50 hover:bg-red-500'}`}
              >
                <span className="text-[8px] uppercase leading-none opacity-80 mt-1">Tập</span>
                <span className="text-[12px] font-black leading-none">{tapHienTai + 1}</span>
              </button>
            </div>
          )}
          ...
          {qualities.length > 0 && (
            <div className="relative flex items-center justify-center">
              ...
              <button onClick={(e) => { e.stopPropagation(); setShowQualityMenu(!showQualityMenu); setShowVolumeSlider(false); setShowTapMenu(false); }} className="w-10 h-10 flex items-center justify-center bg-black/50 rounded-full text-white hover:bg-yellow-500 hover:text-black transition-colors backdrop-blur-md shadow-lg font-bold text-[9px] uppercase">{currentQuality === -1 ? 'AUTO' : qualities.find(q => q.index === currentQuality)?.label || 'AUTO'}</button>
            </div>
          )}
          ...
          <button onClick={(e) => { e.stopPropagation(); resetTimer(); const s = !isThuyetMinh; setIsThuyetMinh(s); if (hlsRef.current) hlsRef.current.audioTrack = s ? 1 : 0; }} className={`p-2.5 rounded-full border transition-all backdrop-blur-md shadow-lg ${isThuyetMinh ? "border-yellow-500 bg-yellow-500/30 text-yellow-400" : "border-white/30 bg-black/50 text-white"}`}>
