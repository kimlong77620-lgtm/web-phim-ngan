...
// 2. PHÒNG CHIẾU (ĐÃ TÍCH HỢP CẢM BIẾN LĂN CHUỘT PC)
function PhongChieu({ phim: initialPhim, onClose, danhSachToanBo }: { phim: Phim; onClose: () => void; danhSachToanBo: Phim[]; }) {
  ...
  // 🎯 CẤU HÌNH VUỐT MÀN HÌNH & LĂN CHUỘT
  const [touchStart, setTouchStart] = useState<{ x: number, y: number } | null>(null);
  const [touchEnd, setTouchEnd] = useState<{ x: number, y: number } | null>(null);
  const minSwipeDistance = 70; 
  const isWheeling = useRef(false); // Cờ khóa con lăn chuột để không bị lướt trôi tuột nhiều phim
  ...
  // 🎯 LÕI CẢM BIẾN LĂN CHUỘT CHO MÁY TÍNH
  const onWheel = (e: React.WheelEvent) => {
    // Nếu phim đang trong quá trình lướt (0.6s), bỏ qua để không bị trôi tuột
    if (isWheeling.current) return;

    const direction = e.deltaY > 0 ? 1 : -1; // Cuộn xuống (1), Cuộn lên (-1)
    ...
    if (nextIndex >= 0 && nextIndex < danhSachToanBo.length) {
      ...
      // Ép màn hình cuộn mượt mà sang phim tiếp theo
      if (el) el.scrollIntoView({ behavior: 'smooth' });

      // Mở khóa con lăn chuột sau khi phim cuộn xong
      setTimeout(() => {
  ...
  return (
    <div 
      ...
      onWheel={onWheel} // 👈 Chốt kích hoạt cảm biến lăn chuột
    >
      <button 
        onClick={handleClose}
